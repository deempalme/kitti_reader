...
namespace kitti {
  ...
  bool kittiCustomObjectsReader::set_dataset(const unsigned int dataset_number){
  ...
  bool kittiCustomObjectsReader::go_to_frame(const unsigned int frame_number){
    obstacles_.clear();
    cylinders_.clear();
    ...
    if(file.is_open()){
      ...
      while(std::getline(file, line)){
        ...
        std::sscanf(line_content, "%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%u,%u,%u",
                    ...
                    &object.position.point.x, &object.position.point.y, &object.position.point.z,
        ...
        object.orientation.axes.x = object.arrow.orientation.axes.x = quaternion.x;
        object.orientation.axes.y = object.arrow.orientation.axes.y = quaternion.y;
        object.orientation.axes.z = object.arrow.orientation.axes.z = quaternion.z;
  ...
  const std::vector<torero::Object> *kittiCustomObjectsReader::obstacles() const{
    return &obstacles_;
  }

  const std::vector<torero::Object> *kittiCustomObjectsReader::cylinders() const{
    return &cylinders_;
  }

  const std::vector<torero::Object> *kittiCustomObjectsReader::fixed_obstacles() const{
    return &obstacles_fixed_;
  }

  const std::vector<torero::Object> *kittiCustomObjectsReader::fixed_cylinders() const{
    return &cylinders_fixed_;
  }

  void kittiCustomObjectsReader::connect_frame(boost::signals2::signal<void (unsigned int)> &signal){
    // Disconnects if previously connected
    if(frame_connection_.connected())
      frame_connection_.disconnect();
    // Makes the new connection
    frame_connection_ =
        signal.connect(boost::bind(&kittiCustomObjectsReader::go_to_frame, this, _1));
  }

  void kittiCustomObjectsReader::connect_dataset(boost::signals2::signal<void (unsigned int)> &signal){
    if(dataset_connection_.connected())
      dataset_connection_.disconnect();
    dataset_connection_ =
        signal.connect(boost::bind(&kittiCustomObjectsReader::set_dataset, this, _1));
  }
  ...
  unsigned int kittiCustomObjectsReader::actual_frame() const{
    return frame_;
  }

  unsigned int kittiCustomObjectsReader::total_frames() const{
